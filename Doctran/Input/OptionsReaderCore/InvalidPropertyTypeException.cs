...
namespace Doctran.Input.OptionsReaderCore
{
    ...
    /// <summary>
    /// This exception should be used to signal to the developer that a property is of an incorrect type to be used with the attribute that was applied.
    /// </summary>
    public class InvalidPropertyTypeException : ApplicationException
    {
        ...
        public InvalidPropertyTypeException(string attributeName, Type expected, Type actual)
            : base(
                $"Error in declaration of 'TOptions'. '{attributeName}' has been applied to a property of type {actual.Name} and must only be applied to properties of type {expected.Name}."
                )
        {
        }
    }
