...
$(document).ready(function () {
    ...
    $("#CallSyntax .syntax").each(function () {
        $(this).find(".content").attr("id", $(this).attr("id")).insertAfter("#CallSyntax .syntax:last");
    });

    // Add the active class to the first call syntax heading.
    $("#CallSyntax h3:first").addClass("active");

    // Hide all of but the first of the syntax contents.
    $("#CallSyntax .content:not(:last)").hide();

    // Add some extra margin between the call syntax lines and the argument/result list.
    $("#CallSyntax .syntax:last").css("margin-bottom", "1em");

    // Add an event when a syntax title is clicked.
    $("#CallSyntax .syntax").each(
	function () {
